...
module Generators
  # constructing an enrollee information of a policy
  class CoverageInformationSerializer
    ...
    def initialize(person, plan_ids = nil)
      @person = person
      @policies = if plan_ids
                    Policy.where({ :enrollees => { "$elemMatch" =>
                                                               { :rel_code => "self",
    ...
    def transform_enrollees
      @policy.enrollees.collect do |enrollee|
        person = enrollee.person
        {
          ...
          is_responsible_party: @policy.responsible_party.present?,
    ...
    def construct_demograhic_info(enrollee)
      member = enrollee.person.authority_member
      {
        ...
        ssn: member.ssn,
        gender_code: (if member.gender == "male"
                        'M'
                      else
    ...
    def construct_segments(enrollee)
      segments = []
      financial_history_dates = policy_history_dates

      financial_history_dates.each do |financial_dates|
    ...
    def append_financial_information(enrollee, financial_dates = nil)
      ...
      return unless (enrollee_coverage_start..enrollee_coverage_end).cover?(financial_dates[0])

      params = if enrollee_coverage_start == enrollee_coverage_end
                 {
                   id: "#{subscriber_m_id}-#{policy_eg_id}-#{start_date}",
      ...
      if enrollee.subscriber?
        ...
        total_premium_amount = calculate_tpa(aptc_credit, financial_dates)
        aptc_amount = calculate_aptc_amount(aptc_credit, total_premium_amount)
        total_responsible_amount = calculate_tra(aptc_credit, total_premium_amount, aptc_amount)
        params.merge!({
    ...
    def calculate_tpa(aptc_credit, financial_dates)
    ...
    def calculate_aptc_amount(aptc_credit, total_premium_amount)
      if aptc_credit.present?
        aptc_credit.aptc.to_f
      elsif @policy.aasm_state == "canceled"
        @policy.applied_aptc.to_f
      else
        find_aptc_amount(total_premium_amount)
      end
    end

    def find_aptc_amount(total_premium_amount)
    ...
    def calculate_tra(aptc_credit, total_premium_amount, aptc_amount)
      if aptc_credit.present?
    ...
    def calculate_total_premium(date)
      ...
      @policy.enrollees.each do |enrollee|
        enrollee_coverage_start = enrollee.coverage_start
        enrollee_coverage_end = enrollee.coverage_end.blank? ? @policy.policy_start.end_of_year : enrollee.coverage_end
        next unless (enrollee_coverage_start..enrollee_coverage_end).cover?(date)
    ...
    def policy_history_dates
      ...
      @policy.aptc_credits.each do |credit|
        ...
        loop_start_dates = add_loop_start_date(loop_start_dates, credit.end_on.next_day) if credit.end_on != @policy.policy_end
      end

...
module IrsGroups
  ...
  class CreateOrUpdateEnrollmentsTaxHouseholds
    ...
    def find_tax_household(tax_household_hbx_id)
      result =
        InsurancePolicies::AcaIndividuals::TaxHouseholds::Find.new.call(
          { scope_name: :by_hbx_id, criterion: tax_household_hbx_id }
        )
    ...
    def find_enrollment(enrollment_hbx_id)
      result =
        InsurancePolicies::AcaIndividuals::Enrollments::Find.new.call(
          { scope_name: :by_hbx_id, criterion: enrollment_hbx_id }
        )
    ...
    def build_enrollments_thh_hash(enr_thh_reference)
      result = {
        tax_household_reference: {
          ...
          max_aptc: {
            cents: enr_thh_reference.tax_household_reference.max_aptc&.cents,
        ...
        hbx_enrollment_reference: {
          hbx_id: enr_thh_reference.hbx_enrollment_reference.hbx_id,
    ...
    def build_enrollment_thh_members_hash(enr_members_thh_members)
      enr_members_thh_members.collect { |enr_member_thh_member| enr_member_thh_member_hash(enr_member_thh_member) }
    end
    ...
    def enr_member_thh_member_hash(enr_member_thh_member)
      {
        hbx_enrollment_member: {
          family_member_reference:
            build_family_member_reference_hash(enr_member_thh_member.hbx_enrollment_member.family_member_reference),
        ...
        tax_household_member: {
          family_member_reference:
            build_family_member_reference_hash(enr_member_thh_member.tax_household_member.family_member_reference)
        },
    ...
    def persist(enrollment)
      ...
      enrollment.tax_households_references.each do |enr_thh_reference|
        ...
        return Failure('Unable to find tax household of enrollment') if tax_household.failure? || enrollment.failure?

        enrollment_tax_household_hash =
          InsurancePolicies::AcaIndividuals::EnrollmentsAndTaxHouseholds::Find.new.call(
            {
              scope_name: :by_enrollment_id_tax_household_id,
              enrollment_id: enrollment.value![:id],
              tax_household_id: tax_household.value![:id]
            }
          )
        ...
        enrollment_tax_household_hash =
          InsurancePolicies::AcaIndividuals::EnrollmentsAndTaxHouseholds::Create.new.call(enr_thh_params_hash)
    ...
    def persist_enrolled_members_tax_household_members(thh_enr_reference, enrollment_tax_household_hash)
      thh_enr_reference.tax_household_members_enrollment_members.each do |thh_member_enr_member|
        ...
        enr_member_thh_member_params.merge!(
          enrollment_tax_household: enrollment_tax_household_hash,
          person: person.value!
        )
        InsurancePolicies::AcaIndividuals::EnrolledMembersAndTaxHouseholdMembers::Create.new.call(
          enr_member_thh_member_params
        )
      end
    end

...
module IrsGroups
  ...
  class GlueBatchRefreshDirector
    ...
    def validate(params)
      errors = []
      errors << "policies required and must be an array" unless params[:policies]
      errors << "policies_to_exclude required and must be an array" unless params[:policies_to_exclude] &&  params[:policies_to_exclude].is_a?(Array)
    ...
    def persist(values)
      ...
      values[:policies].each do |policy_id|
        if values[:policies_to_exclude].hash_key?(policy_id)

...
# TODO: Uncomment specs once create_user_fee aca_entities branch is released to aca entities trunk
...
#
#   context 'Given a new customer/initial transaction, assistance_year and start/end dates' do
#     let(:today) { Date.today }
#     let(:assistance_year) { today.year }
#     let(:start_on) { Date.new(today.year, 1, 1) }
#     let(:end_on) { Date.new(today.year, 12, 31) }
#     let(:new_customer_transaction) { jetson_initial_transaction }
#     let(:customer_hbx_id) { new_customer_transaction[:customer][:hbx_id] }
#
#     it 'should publish an initial enrollment event' do
#       expect(::UserFees::Customer.all.size).to eq 0
#       result = subject.call(new_customer_transaction)
#
#       expect(result.success?).to be_truthy
#       expect(result.success.first.success.payload.dig(:old_state, :customer)).to eq Hash.new
#       expect(result.success.first.success.payload.dig(:new_state, :customer, :hbx_id)).to eq customer_hbx_id
#     end
#
#     context 'Given an existing customer' do
